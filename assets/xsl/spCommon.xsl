...
<xsl:stylesheet version="1.0" 
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<a class="spLienInterne" xml:lang="{$lang}" lang="{$lang}">
					...
					<xsl:attribute name="href">
						<xsl:text>index.php?xml=</xsl:text>
						<xsl:value-of select="$href"/>
						<xsl:text></xsl:text>
						<!-- <xsl:text disable-output-escaping="yes"><![CDATA[&]]>xsl=spMainNoeud.xsl</xsl:text> -->
					</xsl:attribute>
					<xsl:copy-of select="$text"/>
				</a>
				<xsl:if test="$audienceTitle != ''">
	...
	<xsl:template name="getSiteLink">
		...
		<xsl:param name="text"/>
		<xsl:param name="lang" select="'fr'"/>
		<a rel="nofollow" class="spLienExterne" xml:lang="{$lang}" lang="{$lang}" target="_blank">
			<xsl:attribute name="title">
				<xsl:value-of select="normalize-space($title)"/>
			</xsl:attribute>
			<xsl:attribute name="href">
				...
				<xsl:when test="$CATEGORIE = 'part'">
					<img width="50" height="50" alt="Particuliers">
						...
						<xsl:attribute name="src">
							...
							<xsl:choose>
								<xsl:when test="$id = 'N19803'"><xsl:text>argent.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19804'"><xsl:text>europe.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19805'"><xsl:text>famille.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19806'"><xsl:text>formation.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19807'"><xsl:text>justice.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19808'"><xsl:text>logement.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19809'"><xsl:text>loisirs.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19810'"><xsl:text>citoyennete.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19811'"><xsl:text>sante.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N19812'"><xsl:text>transport.png</xsl:text></xsl:when>
								<xsl:otherwise><xsl:text>trsp.gif</xsl:text></xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
					</img>
				</xsl:when>
					...
					<img width="50" height="50" alt="Entreprises">
						...
						<xsl:attribute name="src">
							...
							<xsl:choose>
								...
								<xsl:when test="$id = 'N24264'"><xsl:text>creation-cession.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N24265'"><xsl:text>fiscalite.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N24266'"><xsl:text>gestion-finance.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N24267'"><xsl:text>rh.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N24268'"><xsl:text>vente-commerce.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'N24269'"><xsl:text>secteurs.png</xsl:text></xsl:when>
	
								<xsl:when test="$id = 'F23961'"><xsl:text>comment-faire.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'F23697'"><xsl:text>comment-faire.png</xsl:text></xsl:when>
								<xsl:when test="$id = 'F23571'"><xsl:text>comment-faire.png</xsl:text></xsl:when>
	...
	<xsl:template name="imageOfAPartie">
		...
		<xsl:if test="$AFF_IMAGES_PARTIE = 'true'">
		...
		<img width="20" height="20" class="entiteImageFloatLeft">
			<xsl:choose>
				...
				<xsl:when test="@type = 'attention'">
					<xsl:attribute name="src">
						<xsl:value-of select="$IMAGES"/>
						<xsl:text>attention.png</xsl:text>
					</xsl:attribute>
					<xsl:attribute name="alt">
