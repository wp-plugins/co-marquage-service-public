...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:key name="dossier" match="Dossier" use="translate(substring(Titre,1,1),'ÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖØÙÚÛÜ','AAAAAAEEEEIIIIOOOOOOUUUU')"/>
	<xsl:variable name="letters" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
   	...
   	<xsl:template match="/Dossiers">
		<div class="spIndexMain">
			...
			<div class="spCenter">
				<h1>
					<xsl:text>Dossiers de A à Z du guide des </xsl:text>
			...
			<xsl:apply-templates select="Dossier[generate-id(.)=generate-id(key('dossier',translate(substring(Titre,1,1),'ÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖØÙÚÛÜ','AAAAAAEEEEIIIIOOOOOOUUUU'))[1])]">
       	...
       	<li>
        	<xsl:choose>
	        	<xsl:when test="key('dossier',$letter)">
	        		<a href="#sp-letter-{$letter}" title="Dossiers commençant par la lettre {$letter}">
			        	<xsl:value-of select="$letter"/>
	        		</a>
	        	</xsl:when>
	        	<xsl:otherwise>
