...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="affServiceEnLigne">
		<xsl:if test="count(ServiceEnLigne) > 0">
			<div class="spPublicationSEL block automore" id="sp-service-en-ligne">
				<h2>
					<i class="fa fa-at"><xsl:text> </xsl:text></i>
					<xsl:call-template name="imageOfAPartie">
					<xsl:with-param name="nom">enligne</xsl:with-param>
					</xsl:call-template>
					<xsl:text>Services en ligne et formulaires</xsl:text>
				</h2>
