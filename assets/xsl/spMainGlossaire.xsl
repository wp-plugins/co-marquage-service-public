...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:key name="glossaire" match="Definition" use="translate(substring(Titre,1,1),'ÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖØÙÚÛÜ','AAAAAAEEEEIIIIOOOOOOUUUU')"/>
	<xsl:variable name="letters" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
   	...
   	<xsl:template match="/Glossaire">
		<div class="spIndexMain">
			...
			<div class="spCenter">
				<h1>
					<xsl:text>Glossaire de A à Z du guide des </xsl:text>
			...
			<xsl:apply-templates select="Definition[generate-id(.)=generate-id(key('glossaire',translate(substring(Titre,1,1),'ÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖØÙÚÛÜ','AAAAAAEEEEIIIIOOOOOOUUUU'))[1])]">
       	...
       	<li>
        	<xsl:choose>
	        	<xsl:when test="key('glossaire',$letter)">
	        		<a href="#sp-letter-{$letter}" title="Définitions commençant par la lettre {$letter}">
			        	<xsl:value-of select="$letter"/>
	        		</a>
	        	</xsl:when>
	        	<xsl:otherwise>
